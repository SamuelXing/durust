...
You keep **SamuelXing/durare** (a Rust SDK for the DBOS
...
deduplicated tracking issues in `SamuelXing/durare`.
...
2. For each item, classify:
   - **kind**: `feature` | `bugfix` | `semantics` | `skip`. Skip docs, CI,
     ...
     the SDK's contract.
   - **parity_relevant**: would durare need to change to match this behavior?
     If no, skip it.
   - **phase**: the `PARITY.md` phase/row it maps to, or `unmapped`.
   - Note its current durare status from PARITY.md (✅/🟡/❌). An item already ✅
...
4. **Ensure labels exist** (ignore "already exists" errors):
   ...
   gh label create kind:bugfix   -c "#D93F0B" --repo SamuelXing/durare 2>/dev/null || true
   gh label create kind:semantics -c "#FBCA04" --repo SamuelXing/durare 2>/dev/null || true
   ```
...
5. **File a tracking issue** for each new parity-relevant item:
   ...
   - **PARITY mapping** — the phase/row and its current durare status.
...
- Prefer merged PRs over open issues as evidence; an open *issue* is only worth
  filing if it documents a real semantic gap/bug durare shares.
- Group near-duplicates (the same feature landing in several SDKs) into **one**
  durare issue, with all upstream links and one marker per upstream item.
...
- Stay within `SamuelXing/durare` for writes; the upstream repos are read-only.
